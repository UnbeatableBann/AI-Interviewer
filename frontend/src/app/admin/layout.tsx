...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-background text-muted-foreground font-mono text-xs animate-pulse">
        Verifying system administrator credentials...
      </div>
    );
  }
  ...
  const navItems = [
    ...
    { label: "Tenant Contexts", href: "/admin/tenants", icon: Building2 },
    { label: "User Management", href: "/admin/users", icon: Users2 },
    { label: "Compliance Audits", href: "/admin/audit-logs", icon: FileCheck2 },
    ...
    <div className="flex min-h-screen bg-background font-sans">
      ...
      <aside className="hidden md:flex w-64 flex-col border-r border-border bg-card">
        <div className="flex h-14 items-center gap-2 px-6 border-b border-border">
          ...
          <span className="font-semibold tracking-tight text-sm">IIP Root Admin</span>
        </div>
        ...
        <header className="flex h-14 items-center justify-between px-6 border-b border-border bg-card">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">
              System Admin Gateway | Root Context Active
            </span>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex md:hidden bg-background/80 backdrop-blur-sm">
            <div className="w-64 bg-card border-r border-border h-full flex flex-col p-6 shadow-xl relative animate-in slide-in-from-left duration-200">
              ...
              <div className="flex items-center gap-2 pb-6 border-b border-border mb-6">
                ...
                <span className="font-semibold text-sm">Admin Gateway</span>
              </div>
