...
export default function RecruiterLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-background text-muted-foreground font-mono text-xs">
        Verifying secure recruiter session...
      </div>
    );
  }
  ...
  const navItems = [
    ...
    { label: "Candidates Center", href: "/recruiter/candidates", icon: Users },
    { label: "Interview Sessions", href: "/recruiter/interviews", icon: Calendar },
    { label: "Assessment Rubrics", href: "/recruiter/templates", icon: FileSpreadsheet },
    { label: "Workspace & RAG Settings", href: "/recruiter/settings", icon: Settings },
  ];
    ...
    <div className="flex min-h-screen bg-background font-sans">
      ...
      <aside className="hidden md:flex w-64 flex-col border-r border-border bg-card">
        <div className="flex h-14 items-center gap-2 px-6 border-b border-border">
          ...
          <span className="font-semibold tracking-tight text-sm">IIP Recruiter Portal</span>
        </div>
        ...
        <header className="flex h-14 items-center justify-between px-6 border-b border-border bg-card">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">
              Tenant Admin Gateway | X-Tenant-ID Active
            </span>
          </div>

          <div className="flex items-center gap-4 font-mono text-[11px]">
            <span className="border border-border px-2 py-0.5 rounded bg-accent text-foreground">
          ...
          <div className="fixed inset-0 z-50 flex md:hidden bg-background/80 backdrop-blur-sm">
            <div className="w-64 bg-card border-r border-border h-full flex flex-col p-6 shadow-xl relative animate-in slide-in-from-left duration-200">
              ...
              <div className="flex items-center gap-2 pb-6 border-b border-border mb-6">
                ...
                <span className="font-semibold text-sm">Recruiter Portal</span>
              </div>
